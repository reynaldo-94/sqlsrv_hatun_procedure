...
' Cambios  :                              
 ...
 - 13-10-2022  REYNALDO CAUCHE - Se elimino el campo RolId y se agrego el campo Actualiza esto es para saber cuando se va a actualizar el campo fechaDescarga            
 - 18-10-2022  FRANCISCO LAZARO - Se modifica formato de fechas para mostrar fecha y hora             
'--------------------------------------------------------------------------------------*/                                                                                       
...
BEGIN                                                   
    DECLARE @TableDerivaciones table(Nombres varchar(500), DocumentoNum varchar(500),                                                   
             ...
             FechaDerivacion  varchar(500), DiaLlamada varchar(50),                                     
    ...
    SELECT ISNULL(P.Nombre + ' ' + P.ApePaterno + ' ' + P.ApeMaterno, ''),     
           ISNULL(P.DocumentoNum, ''),     
           ISNULL(DD.Direccion, ''),                                                                                        
           ISNULL(CONVERT(varchar, ECD.Fecha, 103) + ' ' + CONVERT(varchar, ECD.Fecha, 108), ''),      
           ISNULL(IIF(@BancoId <> 12, O.CodTopaz, O.IdOficina), O.IdOficina)[CodTopaz], 
           ISNULL(O.Nombre, ''),              
           ISNULL(SOL.Analista,''),                                                                               
           ISNULL(SOL.IdAnalista, 0),                                                                                                       
           ISNULL((Select top 1 Nombre from SGF_ADV where AdvId=EC.AdvId AND EsActivo=1),''),                               
           ISNULL(SOL.MontoPropuesto, 0),                                                                                               
           ISNULL(dbo.FN_Fechas_Derivacion(ECD.ExpedienteCreditoId), ''),                                                                                   
           ECD.ExpedienteCreditoId,                                                            
           (SELECT top 1 CONVERT(varchar, Fecha, 103) + ' ' + CONVERT(varchar, Fecha, 108) FROM SGF_ExpedienteCreditoDetalle         
           ...
           ISNULL(S.Nombres + ' ' + S.ApePaterno + ' ' + S.ApeMaterno, ''),              
           ISNULL(S.DocumentoNum, ''),              
           ...
           ISNULL(CONVERT(varchar, EC.FechaDescarga, 103) + ' ' + CONVERT(varchar, EC.FechaDescarga, 108), '') [FechaDescarga],              
           ISNULL(REPLACE(REPLACE(REPLACE(REPLACE(EC.ObsDerivacion, Char(9), ''), Char(10), ''), Char(13), ''), Char(59), ''), ''),                                                                       
           ISNULL(ECD.Fecha, ''),       
           ISNULL(P.DiaLlamada, ''),                                                         
           ISNULL(P.Horario, ''),                         
    ...
    BEGIN                                                   
        ...
        SET FechaDescarga = CONVERT(varchar, dbo.getdate(), 103) + ' ' + CONVERT(varchar, dbo.getdate(), 108)          
        WHERE ActualizaFecha = 'SI'         
    END                     
