...
' Cambios  :                                                           
  ...
  - 09/10/2023 - Reynaldo Cauche - Se agrego isActie = 1 para motivo rechazo (26)     
'--------------------------------------------------------------------------------------*/                                                                                              
   ...
   END                                                                                                     
    ...
    BEGIN                
     DECLARE @XRolId INT = ISNULL((SELECT CargoId FROM SGF_USER WHERE UserId = @EmpleadoId), 0);         
        ...
        WHERE Activo = 1  AND        
             ((@XRolId in (4, 11, 1, 56) or (@XRolId = 52 and @XRegionZonaId in (6))) or        
     ((@XRolId not in (4, 11, 1, 56) and not (@XRolId = 52 and @XRegionZonaId in (6))) and BancoId != 13))       
    END             
