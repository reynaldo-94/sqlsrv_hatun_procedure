SELECT solicitudid, * FROM sgf_Expedientecredito where bancoID = 8
SELECT solicitudid,bancoid, * FROM sgf_Expedientecredito where ExpedienteCreditoId = 707343
select IdOficina,* from sgf_solicitud where solicitudid = 382044

EXEC SGC_SP_Derivaciones_Banco_L 4,2022,0,0,0,20,''
...
'--------------------------------------------------------------------------------------*/                 
ALTER PROCEDURE SGC_SP_Derivaciones_Banco_L       
(@Mes int = 1,                 
 @Anio int = 2017,                 
 @LocalId int = 0,   
 @BancoId int,   
