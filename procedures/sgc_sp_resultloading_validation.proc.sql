...
BEGIN                      
    ...
    BEGIN                      
        ...
        SELECT ISNULL(EXPC.BancoId, 0) [BancoId],                      
               ...
               IIF(EXPC.BancoId <> @BancoId,     
      'OP no pertenece a Banco',                       
                   IIF((UPPER(B.Estado) = 'DESEMBOLSO' and EXPC.EstadoProcesoId not in (5, 9, 10, 16)) or                       
                       ...
                       (UPPER(B.Estado) in ('EVALUACION', 'PRE-PRESTAMO', 'REVISION', '') and EXPC.EstadoProcesoId not in (5)),    
        'OP se encuentra en estado ' + EST.NombreLargo,                      
                          IIF(UPPER(B.Estado) in ('EVALUACION', 'PRE-PRESTAMO', 'REVISION', '') and EXCD.ItemId IS NOT NULL,    
        'Comentario ya Agregado anteriormente',                     
                             IIF(UPPER(B.Estado) not in ('DESEMBOLSO', 'CERRADO', 'RECHAZADO', 'EVALUACION', 'PRE-PRESTAMO', 'REVISION', ''),    
        'Estado no válido',                     
                                IIF(EXPC.ExpedienteCreditoId is NULL, 'NOP no válido',    
        'Correcto')    
     ))))[Observacion],                       
...
--                            IIF(EXPC.ExpedienteCreditoId is NULL, 0, 1)))))[EsValido]          
        ...
        OFFSET @Pagina * IIF(@Tamanio <= 0, @TABLEEXPCRED_COUNT, @Tamanio) ROWS FETCH NEXT IIF(@Tamanio <= 0, @TABLEEXPCRED_COUNT, @Tamanio) ROWS ONLY;                    
    END        
    ELSE IF @BancoId = 13 -- Alfin        
    BEGIN        
        SET @Success = 1        
      
        SELECT       
               ISNULL(EXPC.BancoId, 0) [BancoId],        
               ISNULL(A.ExpedienteCreditoId, 0) [ExpedienteCreditoId],        
               iif((UPPER(B.Estado) in ('RECHAZADO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0),9    
                  ,iif((UPPER(B.Estado) in ('DESISTIO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0),10    
                     ,iif(UPPER(B.Estado) in ('ACTIVADO') and EXPC.EstadoProcesoId in(5,9,10,16) and isnull(EXPC.EnObservacion,0) = 0 ,8    
                        ,ISNULL(EXPC.EstadoProcesoId, 0))))[EstadoProcesoId],    
               iif((UPPER(B.Estado) in ('RECHAZADO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0),'Activado'    
                  ,iif((UPPER(B.Estado) in ('DESISTIO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0),'Desistió'    
                     ,iif(UPPER(B.Estado) in ('ACTIVADO') and EXPC.EstadoProcesoId in(5,9,10,16) and isnull(EXPC.EnObservacion,0) = 0 ,'Rechazado1'    
                         ,ISNULL(EST.NombreLargo, '')))) [EstadoProcesoNombre],        
               ISNULL(B.Estado, '') [EstadoBancoNombre],                    
               ISNULL(PER.DocumentoNum, '') [DocumentoNum],                    
               ISNULL(PER.Nombre + ' ' + PER.ApePaterno + ' ' + PER.ApeMaterno, '') [NombreCompleto],     
               IIF(EXPC.BancoId <> @BancoId,'OP no pertenece a Banco',    
                   IIF(EXPC.ExpedienteCreditoId is NULL, 'NOP no válido',    
                      IIF(UPPER(B.Estado) not in ( 'RECHAZADO', 'DESISTIO', 'EVALUACION', 'ACTIVADO', ''),'Estado no válido'    
                          ,IIF((UPPER(B.Estado) ='ACTIVADO' and EXPC.EstadoProcesoId =8)    
                          or (UPPER(B.Estado) ='RECHAZADO' and EXPC.EstadoProcesoId = 9)     
                          or (UPPER(B.Estado) = 'DESISTIO' and EXPC.EstadoProcesoId = 10),'OP se encuentra en estado ' + EST.NombreLargo    
                              ,IIF(UPPER(B.Estado) in ('EVALUACION') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0,'Correcto'    
                                 ,IIF((UPPER(B.Estado) in ('RECHAZADO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0)    
                                 or (UPPER(B.Estado) in ('DESISTIO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0),'Correcto'    
                                    ,IIF(UPPER(B.Estado) in ('ACTIVADO') and EXPC.EstadoProcesoId in(5,9,10,16) and isnull(EXPC.EnObservacion,0) = 0,'Correcto'    
                                        ,'OP se encuentra en estado ' + EST.NombreLargo ))))))) [Observacion],    
               IIF(EXPC.BancoId <> @BancoId, 0,    
                   IIF(EXPC.ExpedienteCreditoId is NULL,0,    
                       IIF(UPPER(B.Estado) not in ( 'RECHAZADO', 'DESISTIO', 'EVALUACION', 'ACTIVADO', ''),0    
                           ,IIF((UPPER(B.Estado) ='ACTIVADO' and EXPC.EstadoProcesoId =8)    
                           or (UPPER(B.Estado) ='RECHAZADO' and EXPC.EstadoProcesoId = 9)     
                           or (UPPER(B.Estado) = 'DESISTIO' and EXPC.EstadoProcesoId = 10),0    
                               ,IIF(UPPER(B.Estado) in ('EVALUACION') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0,1    
                                   ,IIF((UPPER(B.Estado) in ('RECHAZADO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0)    
                                   or (UPPER(B.Estado) in ('DESISTIO') and EXPC.EstadoProcesoId = 5 and isnull(EXPC.EnObservacion,0) = 0),1    
                                      ,IIF(UPPER(B.Estado) in ('ACTIVADO') and EXPC.EstadoProcesoId in(5,9,10,16) and isnull(EXPC.EnObservacion,0) = 0,1    
                                          ,0)))))))[EsValido]     
            FROM @TABLEEXPCRED A                     
               LEFT JOIN @TABLEESTADO B ON A.Id = B.Id        
               LEFT JOIN SGF_ExpedienteCredito EXPC ON A.ExpedienteCreditoId = EXPC.ExpedienteCreditoId        
