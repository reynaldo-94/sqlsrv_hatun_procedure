...
'--------------------------------------------------------------------------------------*/                               
ALTER PROCEDURE [dbo].[SGC_SP_ResultLoading_Validation_Alfin]              
(@BancoId int,              
...
BEGIN              
    ...
    BEGIN
        ...
        SELECT ISNULL(EXPC.BancoId, 0) [BancoId],
               ...
               ISNULL(PER.Nombre + ' ' + PER.ApePaterno + ' ' + PER.ApeMaterno, '') [NombreCompleto],
               IIF(EXPC.BancoId <> @BancoId, 'OP no pertenece a Banco',
                 IIF(UPPER(B.Estado) not in ('DESEMBOLSADO', 'RECHAZADO', 'DESISTIO', 'EVALUACION', 'PENDIENTE', 'VACIO', ''), 'Estado no válido',
                    IIF(EXPC.ExpedienteCreditoId is NULL, 'NOP no válido', 
                        IIF(UPPER(B.Estado) = 'DESEMBOLSADO' and EXPC.EstadoProcesoId not in (5, 9, 10, 16), 'OP se encuentra en estado ' + EST.NombreLargo, 'Correcto')))
                )  [Observacion],
               IIF(EXPC.BancoId <> @BancoId, 0,
                 IIF(UPPER(B.Estado) not in ('DESEMBOLSADO', 'RECHAZADO', 'DESISTIO', 'EVALUACION', 'PENDIENTE', 'VACIO', ''), 0,
                    IIF(EXPC.ExpedienteCreditoId is NULL, 0, 
                        IIF(UPPER(B.Estado) = 'DESEMBOLSADO' and EXPC.EstadoProcesoId not in (5, 9, 10, 16), 0, 1)))
                )  [EsValido]
        FROM @TABLEEXPCRED A             
        LEFT JOIN @TABLEESTADO B ON A.Id = B.Id
        LEFT JOIN SGF_ExpedienteCredito EXPC ON A.ExpedienteCreditoId = EXPC.ExpedienteCreditoId
