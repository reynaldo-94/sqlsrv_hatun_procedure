...
BEGIN TRY                                         
    BEGIN TRANSACTION                                                      
        ...
        SET @DatosLaboralesId = (SELECT TOP (1) ISNULL(DatosLaboralesId,0) FROM SGF_DatosLaborales WHERE PersonaId = @PersonaId)

        SET @documentoNum = (select p.DocumentoNum from SGF_DatosLaborales dl inner join SGF_Persona p on p.PersonaId = dl.PersonaId where dl.DatosLaboralesId = @DatosLaboralesId); 
        ...
        -- set @ubigeo = (select isnull(count(ISNULL(sdd.Ubigeo,0)),0) from SGF_DatosLaborales dl INNER JOIN SGF_DatosDireccion sdd ON dl.PersonaId = sdd.PersonaId where dl.DatosLaboralesId = @DatosLaboralesId)
        set @ubigeo = (select isnull(count(ISNULL(sdd.Ubigeo,0)),0) from SGF_DatosLaborales dl INNER JOIN SGF_DatosDireccion sdd ON dl.PersonaId = sdd.PersonaId where dl.DatosLaboralesId = @DatosLaboralesId)                                                    
        set @direccion = (select isnull(count(ISNULL(sdd.Direccion,0)),0) from SGF_DatosLaborales dl INNER JOIN SGF_DatosDireccion sdd ON dl.PersonaId = sdd.PersonaId where dl.DatosLaboralesId = @DatosLaboralesId)                                                    
        set @referencia = (select isnull(count(ISNULL(sdd.Referencia,0)),0) from SGF_DatosLaborales dl INNER JOIN SGF_DatosDireccion sdd ON dl.PersonaId = sdd.PersonaId where dl.DatosLaboralesId = @DatosLaboralesId) 
        set @tipodireccion = (select isnull(count(ISNULL(sdd.TipoDireccionId,0)),0) from SGF_DatosLaborales dl INNER JOIN SGF_DatosDireccion sdd ON dl.PersonaId = sdd.PersonaId where dl.DatosLaboralesId = @DatosLaboralesId) 
