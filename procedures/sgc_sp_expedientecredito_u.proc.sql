...
' Cambios:                    
  ...
  27/05/2023 - REYNALDO CAUCHE - Se cambió la forma de obtener de las variables @DatosDireccionId, @DatosLaboralesId, @TipoPersonaId 
...
BEGIN                            
    ...
    BEGIN TRY                                      
        ...
        SET @DatosDireccionId = (SELECT DatosDireccionId from SGF_DatosDireccion where PersonaId = @PersonaId and TipoDireccionId = 1)  
        SET @DatosLaboralesId = (SELECT DatosLaboralesId FROM SGF_DatosLaborales where PersonaId = @PersonaId)  
        SET @TipoPersonaId = (select TOP(1) ev.TipoPersonaId 
                            ...
                            where ExpedienteCreditoId = @PersonaId) 
	    ...
	    Begin                                    
	        ...
	        Begin                                    
	           set @SolicitudId=(select max(SolicitudId)+1 from SGF_Solicitud)                                    
	               INSERT INTO SGF_Solicitud       
	                  (SolicitudId,BancoId,PersonaId,FechaSolicitud,MonedaId,FrecuenciaPagoId,MontoPropuesto,MontoMaterialPro,MontoEfectivoPro,EstadoSolicitudId,UserIdCrea,FechaCrea)                                    
	               VALUES       
	                  (@SolicitudId,@BancoId,@PersonaId,dbo.getdate(),1,3,@MontoPropuesto,@MontoMaterialPro,@MontoEfectivoPro,1,@UserIdActua,dbo.getdate())                                    
	    ...
	    BEGIN                          
	       SET @ItemId=(Select max(ItemId)+1 from SGF_ExpedienteCreditoDetalle WHERE ExpedienteCreditoId = @ExpedienteCreditoId)                          
	         INSERT INTO SGF_ExpedienteCreditoDetalle       
	           (ExpedienteCreditoId, ItemId, ProcesoId, Fecha, Observacion, UsuarioId)                          
	         VALUES       
	           (@ExpedienteCreditoId, @ItemId, @EstadoProcesoId, dbo.getdate(), @ObservacionMensaje, @UserIdActua)                          
	    END                    
